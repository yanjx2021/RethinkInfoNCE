...
# -*- coding: utf-8 -*-
"""
datascale.py: 2x2 line plots over data scaling (subsets) for one or more models.

Outputs (to --out-dir):
  1) datascale_vs_datasize.png/.pdf  (x = datasize; log-x)
     - Adds a GLOBAL dashed log-log fit + R^2 ONLY for metrics with yscale=="log".
  2) datascale_vs_mrr.png/.pdf       (x = mrr; linear-x)
     - No fit lines.
...
Example:
  python datascale.py \
    ...
    --models google_bert/uncased_L-12_H-768_A-12 \
    --subsets 100 3000 6000 10000 30000 60000 100000 502939 \
    --full-subset 502939 \
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        ...
        nargs="+",
        type=int,
        default=[100, 3000, 6000, 10000, 30000, 60000, 100000, 502939],
    )
    ...
    ap.add_argument("--wspace", type=float, default=0.38)
...
def plot_2x2_lines(
    ...
    *,
    ...
    for ax, cfg in zip(ax_list, METRICS_CFG):
        ...
        all_x_for_fit: List[float] = []
        all_y_for_fit: List[float] = []
        ...
        for model in models_sorted:
            ...
            if add_global_fit and yscale == "log":
                all_x_for_fit.extend(list(xs))
                all_y_for_fit.extend(list(ys))

        if add_global_fit and yscale == "log" and len(all_x_for_fit) >= 2:
            plot_global_loglog_fit(
                ax,
                np.asarray(all_x_for_fit, dtype=np.float64),
                np.asarray(all_y_for_fit, dtype=np.float64),
                r2_pos=r2_pos,
                r2_font=r2_font,
            )
...
def main() -> None:
    ...
    out1 = args.out_dir / "datascale_vs_datasize.png"
    plot_2x2_lines(
        metrics_dict,
        ...
        models=args.models,
        subsets=args.subsets,
        x_getter=lambda _m, subset: float(subset),
        xlabel="datasize",
        xscale="log",
        out_path=out1,
    ...
    out2 = args.out_dir / "datascale_vs_mrr.png"
    plot_2x2_lines(
        metrics_dict,
        model_color_map=model_color_map,
        models_sorted=models_sorted,
        models=args.models,
        subsets=args.subsets,
        x_getter=lambda m, _subset: m.get("mrr", None),
